...
io.on("connection", (socket) => {
  ...
  socket.on("mark-read", async ({ applicationId, userId }) => {
    if (applicationId) {
      // Notify other users in the chat room (for ticks)
      socket.to(applicationId).emit("messages-read", { applicationId, readBy: userId });

      // Notify ALL of the reader's components (Navbar, Sidebar) to update counts
      io.in(`notifications-${userId}`).emit("messages-read", { applicationId, readBy: userId });

      // Find the application to get the other participant and notify them too
      try {
        const application = await ApplicationModel.findById(applicationId)
          .populate({
            path: 'opportunity',
            populate: { path: 'ngo', select: '_id' }
          })
          .populate('volunteer', '_id');
        ...
        if (application && application.opportunity && application.opportunity.ngo && application.volunteer) {
          ...
          // Determine who the OTHER user is (the sender whose messages were just read)
          const otherUserId = userId === ngoId ? volunteerId : ngoId;

          // Notify the sender that their messages were read
          io.in(`notifications-${otherUserId}`).emit("messages-read", { applicationId, readBy: userId });

          console.log(`Messages in chat ${applicationId} marked as read by ${userId}, notified sender ${otherUserId}`);
        }
      } catch (err) {
        console.error("Error notifying message sender:", err);
      }

      console.log(`Messages in chat ${applicationId} marked as read by ${userId}`);
    }
  });
