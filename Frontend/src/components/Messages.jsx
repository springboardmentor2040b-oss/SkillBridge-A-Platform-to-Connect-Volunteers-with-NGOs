...
import { MdSend, MdInsertEmoticon, MdAttachFile, MdArrowBack, MdMoreVert, MdDelete, MdChat } from "react-icons/md";
...
function Messages() {
  ...
  // Get unread count for empty state header
  ...
  useEffect(() => {
    if (!applicationId && isAuthenticated) {
      ...
      socket.connect();
      joinNotifications(currentUserId);
      ...
      return () => {
        ...
        leaveNotifications(currentUserId);
      };
    }
  }, [applicationId, isAuthenticated, currentUserId, fetchUnreadCount]);
  ...
  const markAsRead = useCallback(async () => {
    ...
    try {
      console.log(`Messages: Marking messages as read for app ${applicationId}`);
      const response = await axios.put(`http://localhost:4001/api/messages/${applicationId}/read`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      console.log(`Messages: Backend response:`, response.data);

      // Emit to socket that messages have been read
      // The backend will broadcast this to all relevant clients
      socket.emit("mark-read", { applicationId, userId: currentUserId });
      console.log(`Messages: Emitted mark-read event for app ${applicationId}, user ${currentUserId}`);
    } catch (err) {
      console.error("Error marking messages as read:", err);
    }
  }, [applicationId, token, currentUserId]);
  // Socket connection
  useEffect(() => {
    if (!applicationId || !isAuthenticated) return;
    ...
    socket.on("receive-message", (data) => {
      ...
      if (String(data.applicationId) === String(applicationId)) {
        setMessages((prev) => [...prev, data]);
        // If current user is viewing this chat, mark it as read
        markAsRead();
      }
    });
    ...
    return () => {
      ...
      // Don't disconnect socket - it's shared across the app for notifications
    };
  }, [applicationId, isAuthenticated, currentUserId, markAsRead]);
  ...
  const handleDeleteChat = useCallback(async () => {
    ...
    try {
      await axios.delete(`http://localhost:4001/api/messages/${applicationId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  if (!applicationId) {
    return (
      <div className="flex flex-col h-full bg-gray-50 relative">
        ...
        {/* HEADER */}
        <div className="h-20 bg-gradient-to-br from-indigo-900 via-purple-800 to-fuchsia-700 flex items-center px-6 border-b shadow-sm absolute top-0 left-0 right-0 z-20">
          <div className="w-14 h-14 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center font-bold text-lg shadow-lg border-2 border-white/20">
          ...
          <div className="ml-4 flex items-center gap-3">
            ...
            {unreadCount > 0 && (
              <span className="bg-orange-500 text-white text-xs font-black rounded-full w-6 h-6 flex items-center justify-center shadow-lg animate-pulse">
                {unreadCount}
              </span>
            )}
          </div>
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 relative">
      {/* HEADER */}
      <div className="h-16 bg-gradient-to-br from-indigo-900 via-purple-800 to-fuchsia-700 flex items-center px-4 border-b shadow-sm flex-shrink-0 relative">
        <button
          onClick={() => navigate("/messages")}
          className="mr-3 p-2 hover:bg-white/10 rounded-full transition-colors lg:hidden text-white"
        >
          <MdArrowBack className="h-5 w-5" />
        </button>
        <div className="w-10 h-10 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center font-bold shadow-md border-2 border-white/20">
          {getAvatarInitial()}
        </div>
        <div className="ml-3 flex-1">
          <p className="text-sm font-semibold text-white">{getChatName()}</p>
              ...
              <div key={msg._id || i}>
                {showDate && (
                  <div className="flex items-center justify-center my-4">
                    <span className="text-xs text-gray-400 bg-white px-3 py-1 rounded-full shadow-sm font-medium">
                ...
                <div className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-[75%] px-4 py-2.5 rounded-2xl shadow-sm relative min-w-[80px]
                    ${isMe
                  ...
                  >
                    ...
                    <div className="absolute bottom-1 right-2 flex items-center gap-1">
                      <span className={`text-[10px] ${isMe ? "text-orange-50" : "text-gray-400"}`}>
                      ...
                      {isMe && (
                        <div className="flex items-center">
                          {msg.read ? (
                            ...
                            <svg className="w-3.5 h-3.5 text-orange-200" fill="currentColor" viewBox="0 0 24 24">
                              <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" />
                            </svg>
                          )}
                        </div>
